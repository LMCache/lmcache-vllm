...
from lmcache_vllm.experimental.vllm_adapter import (
    init_lmcache_engine,
...
try:
    from vllm.inputs import DefaultPromptTokenizingMode, SamplingParams, TokensPrompt
    from vllm.outputs import RequestOutput

    logger.debug("Using TokensPrompt from vllm.inputs")
except ImportError:
    from vllm.sampling_params import SamplingParams

    try:
        from vllm.utils import TokensPrompt

        logger.debug("Using TokensPrompt from vllm.utils")
    except ImportError:
        logger.debug(
            "Using TextTokensPrompt from vllm.entrypoints.openai.serving_engine as TokensPrompt"
        )
    # Try to import FinishReason from different locations
    try:
        from vllm.outputs import RequestOutput, FinishReason
    except ImportError:
        # In vLLM 0.8.4, FinishReason might be in vllm.outputs.request_output
        try:
            from vllm.outputs import RequestOutput
            from vllm.outputs.request_output import FinishReason

            logger.debug("Imported FinishReason from vllm.outputs.request_output")
        except ImportError:
            # Define a fallback FinishReason enum if it can't be imported
            ...
            logger.debug("Using fallback FinishReason enum")
    DefaultPromptTokenizingMode = None  # Not available in older vLLM

try:
    from vllm.multimodal import MultiModalKwargs

    logger.debug("Using MultiModalKwargs from vllm.multimodal")
except ImportError:
    try:
        from vllm.inputs import MultiModalInputs as MultiModalKwargs

        logger.debug("Using MultiModalInputs from vllm.inputs as MultiModalKwargs")
    except ImportError:
        # Define a dummy class for compatibility
        @dataclasses.dataclass
        class MultiModalKwargs:
            """Dummy class for MultiModalKwargs"""

            def __init__(self, *args, **kwargs):
                pass

        logger.debug("Using dummy MultiModalKwargs")
...
def new_execute_model(
    ...
    try:
        ...
        if (
            ...
            and model_input.attn_metadata.blend_metadata.selected_token_indices
            is not None
        ):
...
def _new_tokenize_prompt(
    ...
    if cache_config.enable_blending:
        ...
        for text_chunk in text_chunk_list:
            force_no_special_tokens = (
                not is_first_chunk and not cache_config.blend_add_special_in_precomp
            )
...
async def _new_tokenize_prompt_async(
    ...
    if cache_config.enable_blending:
        ...
        for text_chunk in text_chunk_list:
            ...
            effective_add_special_tokens = not force_no_special_tokens
            encoded = await tokenizer.encode_async(
                request_id=request_id,
                prompt=text_chunk,
                lora_request=lora_request,
                add_special_tokens=effective_add_special_tokens,
...
def _new_normalize_prompt_text_to_input(
    ...
    if cache_config.enable_blending:
        ...
        for text_chunk in text_chunk_list:
            ...
            effective_add_special_tokens = (
                add_special_tokens and not force_no_special_tokens
            )
            if truncate_prompt_tokens is None:
                encoded = tokenizer(
                    text_chunk, add_special_tokens=effective_add_special_tokens
                )
            else:
                encoded = tokenizer(
                    text_chunk,
            ...
            blend_indices.append(current_idx)
            is_first_chunk = False
        text_tokens_prompt = self._validate_input(request, input_ids, input_text)
        if len(blend_indices) > 0:
            blend_indices.pop()
...
def new_tokenizer_group_encode(
    self,
    prompt: str,
...
async def new_tokenizer_group_encode_async(
    self,
    prompt: str,
    request_id: Optional[str] = None,
    lora_request: Optional[LoRARequest] = None,
...
async def new_create_chat_completion(
    ...
    if (
        not isinstance(tokenizer, MistralTokenizer)
    ...
    try:
        ...
        if hasattr(request, "to_sampling_params"):
            ...
            sampling_params = SamplingParams(
                ...
                or self.max_model_len - len(prompt_inputs["prompt_token_ids"]),
